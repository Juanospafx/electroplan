...
async function captureSessionCookie() {
  try {
    const check = await fetch(`${SERVER}/api/api.php`, {
      method: 'POST',
      credentials: 'include',
      body: new URLSearchParams({ action: 'get_projects_list' })
    });
    if (check.ok) await Preferences.set({ key: 'session_active', value: 'true' });
  } catch {}
}
...
async function loadOfflineProjects() {
  ...
  container.innerHTML = projects.map(p => `
    <div class="offline-project-card">
      <i class="fas fa-folder-open" style="color:#fb5a3a; font-size:1.5rem;"></i>
      <div>
...
function showOfflineBanner() {
  ...
  if (!banner) {
    ...
    banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:rgba(239,68,68,0.9);color:white;text-align:center;padding:10px;font-size:0.85rem;font-weight:600;backdrop-filter:blur(10px);';
